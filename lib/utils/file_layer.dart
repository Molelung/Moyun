...
class FileLayer {
  ...
  static Future<bool> copyFromExternalLocation(
    ...
    return _copyStream(
        File(externalFile), File(join(internalFolder, outputFile)),
        onProgress: onProgress);
  }
  ...
  static Future<bool> copyToExternalLocation(
    ...
    return _copyStream(
        File(localFile), File(join(externalFolder, outputFile)),
        onProgress: onProgress);
  }

  /// Streams [source] into [destination], reporting progress in 5% steps.
  static Future<bool> _copyStream(File source, File target,
      {Function(double percent)? onProgress}) async {
    ...
    final writeSink = target.openWrite();

    var transferredSize = 0;
    var lastReportedProgress = 0.0;

    await for (List<int> chunk in source.openRead()) {
      writeSink.add(chunk);
