        ...
        when (call.method) {
            "copyFromExternal" -> {
                CoroutineScope(Dispatchers.IO).launch {
                    try {
                        val fileUri = call.argument<String>("src")!!.toUri()
                        val dest = call.argument<String>("dest")!!
                        val transferId = call.argument<String>("transferId")!!

                        // Get file size for progress calculation
                        val totalSize = DocumentFile.fromSingleUri(context, fileUri)?.length() ?: 0L
            ...
            "copyToExternal" -> {
                CoroutineScope(Dispatchers.IO).launch {
                    try {
                        ...
                        val dir = DocumentFile.fromTreeUri(context, treeUri.toUri())
